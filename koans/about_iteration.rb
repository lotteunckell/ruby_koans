...
class AboutIteration < Neo::Koan
  ...
  def test_select_selects_certain_items_from_an_array
    ...
    even_numbers = array.select { |item| (item % 2).zero? } # == 0 }
    assert_equal [2, 4, 6], even_numbers

    # NOTE: 'find_all' is another name for the 'select' operation
    more_even_numbers = array.find_all { |item| (item % 2).zero? } # == 0 }
  ...
  def test_inject_will_blow_your_mind
    ...
    #
    # it itterates through the Array and accumulates it into one result using a
    # start value -> inject(start)
  end
  ...
  #
  # Because using do the file is automatically closed after the block is
  # executed. Otherwise we'd have to close it manually.
end
